...
p_load(rstan, doParallel, plyr, ggplot2, glm2)
...
# do it! #
##########
cl <- makeCluster(36, outfile = "powerSim4varNonAdaptive_out.txt")
...
print(format(Sys.time()-startTime))

outname <- sprintf("powerSim4varNonAdaptive_output_%s.Rds",
                   format(Sys.time(), '%Y%m%d%H%M%S'))
saveRDS(output, outname)

# save workspace in case of future problems
#save.image(file = sprintf("powerSim4varNonAdaptive_workspace_%s.Rdata", 
