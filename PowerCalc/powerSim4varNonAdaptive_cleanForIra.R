...
#######################################################################
{
  ...
  # nC <- output$nC[i]
  # nD <- output$nD[i]
  # cauchyScale <- output$cauchySacle[i]
  # diff <- output$diff[i]
  # sA <- output$sA[i]
  # sB <- output$sB[i]
  # sC <- output$sC[i]
...
#################################################
# for each design, summarize across simulations #
#################################################
MDEs <- aggregate(MDE~N+nA,           
                  #na.action=NULL, # Note: what to do when there are no effects that we can
                  data=output,     #       be 80% sure of detecting at the .05 level?
                  mean)            # Note: could use median here, but it obscures the issue
########
# plot #
########
